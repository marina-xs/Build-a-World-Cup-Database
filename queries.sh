...
#echo "$($PSQL "SELECT DISTINCT t.name, teams.name FROM (SELECT * FROM teams) AS t RIGHT JOIN games ON t.team_id = games.winner_id LEFT JOIN teams on games.opponent_id = teams.team_id WHERE year = 2014 AND round= ‘Eighth-Final’”)”
#(上一种方法最后输出的是分了两列的)
#echo "$($PSQL "SELECT DISTINCT name FROM (SELECT year, round, winner_id FROM games) AS w INNER JOIN teams ON w.winner_id = teams.team_id INNER JOIN games ON teams.team_id = games.opponent_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' ORDER BY name")"
